...
export default function Index() {
  ...
  const [isBmsTheme, setIsBmsTheme] = useState(true);
  ...
  const calculations = useMemo(() => {
    ...
    const impressionsFromMedia = availableMediaBudget > 0 && grossCpmBrl > 0
      ? (availableMediaBudget / grossCpmBrl) * 1000
      : 0;
    const impressionsFromCpm = rawSellingCpm > 0
      ? (totalBudget / rawSellingCpm) * 1000
      : 0;
    const validator = impressionsFromCpm - impressionsFromMedia;
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <img src={bmsLogo} alt="BMS Logo" className="w-20 h-20 object-contain" />
              <div>
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <ValidationBadge 
                ...
                mathProof={{
                  ...
                  rawSellingCpm: calculations.rawSellingCpm,
                  impressionsFromCpm: calculations.impressionsFromCpm,
                  impressionsFromMedia: calculations.impressionsFromMedia,
                  validator: calculations.validator,
                }}
              />
              <Button 
                variant="outline" 
                size="sm"
                onClick={resetValues}
                className={`gap-2 ${isBmsTheme ? 'border-gray-400 text-gray-700 hover:bg-white/20 hover:text-gray-900' : 'border-border hover:bg-muted hover:text-foreground'}`}
              >
              ...
              <Sheet>
                <SheetTrigger asChild>
                  <Button 
                    variant="ghost" 
                    size="icon"
                    className={isBmsTheme ? 'text-white hover:bg-white/20' : ''}
                  >
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        updateValue("exchangeRate", rate);
                        toast({
                          title: "Rate Updated",
                          description: `Commercial USD: R$ ${rate.toFixed(2)}`,
                        });
                      } catch (error) {
                        toast({
                          title: "Error fetching rate",
                          description: "Could not fetch current rate.",
                          variant: "destructive",
                        });
                      }
                    }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="p-6 bg-gradient-card border-border shadow-card animate-slide-up" style={{ animationDelay: "0.3s" }}>
                ...
                <BudgetChart 
                  ...
                  onHoverChange={setHighlightedIndex}
                />
              </Card>
